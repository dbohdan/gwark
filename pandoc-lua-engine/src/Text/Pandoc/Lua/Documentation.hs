...
-- gwark fork: use the Markdown reader (which subsumes commonmark
-- via extension flags) since the CommonMark reader is gone.
...
parseCommonMark txt =
  let exts = extensionsFromList
      ...
      result = runPure $ do
        Pandoc _ blks <- readMarkdown (def {readerExtensions = exts}) txt
