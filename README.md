...
- any other reader (DOCX, EPUB, AsciiDoc, RST, Org, Typst, ipynb, …)
  or writer (LaTeX, DOCX, EPUB, JATS, ConTeXt, …)
- PDF output

The cabal package is renamed to `gwark`; downstream consumers must
update their `build-depends` accordingly. The CLI executable is still
named `pandoc` so existing scripts that shell out to it keep working.

Forked from upstream commit `12051ec` (release 3.9.0.2 head of `main`).

For the full-featured pandoc, see <https://github.com/jgm/pandoc>.

---

The remainder of this file is the original upstream README, kept for
